...
public class App {
	...
	private static void employeesJoinedAfterYear(EntityManager em, Scanner sc) {
		...
		Query q = em.createQuery("from Employee where joiningYear > :year");
	...
	private static void employeesOrderedBySalary(EntityManager em) {
		Query q = em.createQuery("from Employee order by salary desc");
		List<Employee> l = q.getResultList();
		for (Employee e : l) {
			System.out.println(e);
		}
	...
	private static void averageSalaryByDept(EntityManager em) {
		Query q = em.createQuery("select department, avg(salary) from Employee group by department");
		List<Object[]> l = q.getResultList();
		for (Object e[] : l) {
			System.out.println(e[0] + " and " + e[1]);
		}
	}

	private static void totalSalaryExpense(EntityManager em) {
		Query q = em.createQuery("select sum(salary) from Employee");
	...
	private static void increaseSalaryByDept(EntityManager em, Scanner sc) {
		...
		em.getTransaction().begin();
		Query q = em.createQuery("update Employee set salary = salary * 1.1 where department = :dept");
		q.setParameter("dept", dept);
	...
	private static void deleteEmployeesWithLowSalary(EntityManager em, Scanner sc) {
		...
		em.getTransaction().begin();
		Query q = em.createQuery("delete from Employee where salary < :salary");
		q.setParameter("salary", salary);
	...
	private static void employeesNameStartsWith(EntityManager em, Scanner sc) {
		System.out.println("Enter letter to find employee: ");
		String letter = sc.nextLine();

		Query q = em.createQuery("from Employee where name like :letter");
	...
	private static void distinctDepartments(EntityManager em) {
		Query q = em.createQuery("select distinct department from Employee");
	...
	private static void employeesGroupedByDept(EntityManager em) {
		Query q = em.createQuery("select e.department, count(e) from Employee e group by e.department");
		List<Object[]> l = q.getResultList();
		for(Object e[] : l) {
			System.out.println("Department: " + e[0] + " in " + e[1] + " employee");
		}
	}

	private static void employeesWithEarliestJoining(EntityManager em) {
		Query earliestYear = em.createQuery("select min(e.joiningYear) from Employee e");
		int x = (int) earliestYear.getSingleResult();
		
		Query q = em.createQuery("from Employee e where e.joiningYear = :x");
		q.setParameter("x", x);
