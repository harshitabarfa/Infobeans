...
public class StudentService {
	...
	public List<Student> findByNameDepartmentAndMarks(String name, String department, Double marks) {
		List<Student> allStudents = getAllStudent();
		List<Student> result = new ArrayList<>();

		for (Student student : allStudents) {
			boolean matchName = (name == null || name.isEmpty()) || student.getName().equalsIgnoreCase(name);

			boolean matchDepartment = (department == null || department.isEmpty())
					|| student.getDepartment().equalsIgnoreCase(department);

			boolean matchMarks = (marks == null || marks == 0) || Double.compare(student.getMarks(), marks) == 0;

			if (matchName && matchDepartment && matchMarks) {
				result.add(student);
			}
		}
		return result;
